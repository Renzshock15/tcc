                            ...
                            <div class="table-responsive">
                                <table class="tables">
                                    ...
                                    <tbody>
                                        ...
                                        <?php if (!empty($data['studentInfoCollege'])) : ?>
                                            <?php foreach ($data['studentInfoCollege'] as $studentInfosCol) : ?>
                                                <tr>
                                                    <td data-label="Enrollee Id">
                                                        <?php echo $studentInfosCol->subject_enrolled_ID; ?>
                                                    </td>
                                                    <td data-label="Full Name">
                                                        <?php echo $studentInfosCol->lname . ', ' . $studentInfosCol->fname . ' ' . $studentInfosCol->mname; ?>
                                                    </td>
                                                    <td data-label="Student No.">
                                                        <?php echo $studentInfosCol->student_id; ?>
                                                    </td>

                                                    <td data-label="Action">
                                                        <a class="btn btn-danger getCollegeStudent" data-id="<?php echo $studentInfosCol->subject_enrolled_ID; ?>" href="" role="button"><i class="fa fa-user mr-1" aria-hidden="true"></i> Add</a>
                                                    </td>
                                                </tr>
...
<script>
    $(document).ready(function() {
        ...
        $('.getCollegeStudent').click(function() {
            var enrolleId = $(this).data("id");
            var subjectId = '<?php echo $data['subjectId'] ?>';
            var subjectSem = '<?php echo $data['subjectTerm'] ?>';



            $.ajax({
                url: '<?php echo URLROOT ?>' + '/Actions/insertDataCol',
                method: 'post',
                data: {
                    enrolleId: enrolleId,
                    subjectId: subjectId,
